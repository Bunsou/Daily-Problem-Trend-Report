...
v2 format per problem:
  {rank}. {icon} [NOVELTY] Problem name [from: source, ...]
     Category: X  •  Score: Y
     Demand: D  •  Monetization: M  •  Buildability: B
     ...
     📌 Novelty note  (only when present)

Removed from v1: buyer_test rationale, solutions block.
Added in v2: [from: SOURCES] attribution tag, potential_customer line.
...
Programmatic callers use deliver(problems, send_telegram=True) directly.
"""
import sys
...
def format_briefing(problems: list["ScoredProblem"]) -> str:
    ...
    n_trends = sum(1 for p in problems if "trends" in (p.get("sources") or []))
    n_reddit = sum(1 for p in problems if "reddit" in (p.get("sources") or []))
    count_word = "opportunity" if len(problems) == 1 else "opportunities"

    lines = []
    lines.append(f"📊 Daily Opportunity Radar — {_today_str()}")
    lines.append("")
    lines.append(
        f"{len(problems)} qualifying {count_word} today "
        f"({n_trends} from Trends, {n_reddit} from Reddit)"
    )
    lines.append("─" * 40)
    lines.append("")
...
def _format_single_problem(rank: int, p: "ScoredProblem") -> list[str]:
    ...
    potential_customer = p.get("potential_customer") or ""

    lines = [
        f"{rank}. {icon} [{novelty_label}] {p['problem_name']} [from: {sources_csv}]",
        f"   Category: {p['category']}  •  Score: {p['opportunity_score']}",
        f"   Demand: {p['demand']}  •  Monetization: {p['monetization']}  •  Buildability: {p['buildability']}",
        "",
        f"   💡 {p['key_insight']}",
        "",
        f"   🎯 Potential customer: {potential_customer}",
        "",
        f"   📝 {p['description']}",
    ]
...
if __name__ == "__main__":
    ...
    if cached is None:
        ...
        print("Run `python -m app.pipeline.orchestrator` first to generate and cache pipeline output.")
