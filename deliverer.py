...
Currently prints to stdout for testing. The next iteration will add
Telegram bot delivery using the Telegram Bot API.
"""
...
def format_briefing(problems: list["ScoredProblem"]) -> str:
    ...
    formatted string will be sent to Telegram in the next iteration.
    
    Args:
        problems: Scored and novelty-enriched problems from the pipeline.
    
    Returns:
        A formatted message string ready to display or send.
    """
    if not problems:
...
def deliver(problems: list["ScoredProblem"]) -> None:
    """
    Deliver the briefing to the user.
    
    Currently prints to stdout. Future versions will send to Telegram.
    """
    message = format_briefing(problems)
    print(message)


# === Dev mode: load cached output and deliver, no AI calls ===
